...
x_in = 298.43# 400.0 # [mm]
y_in = -157.1923 # 50.0  # [mm]0.29843,-0.157192,0.403154
z_in =  403.154# 300.0  # [mm] 0.119557,-0.262372,0.402319
...
###################
#X,Y=spiral()
y_teach = np.transpose(np.load("/home/franka/palani_ws/src/codes/scripts/VelocityPlanning_DMP_FL/Python_code/path_generation/test1.npz",'r+b')['arr_0'])
X=y_teach[:,0]
Y=y_teach[:,1]
...
plt.legend()
plt.show()
np.savez('radius_curv',[radius_curv])
...
plt.show()
plt.plot(vel*1000,label="actual velocity applied")
plt.plot(velocity,'r',label="calculated velocity")
plt.legend()
#plt.show()
############################################
